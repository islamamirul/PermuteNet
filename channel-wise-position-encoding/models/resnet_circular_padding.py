...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = conv2d_circular(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
...
class ResNetPositionCircular(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, num_location=9):
        super(ResNetPositionCircular, self).__init__()
        self.in_planes = 64

        self.conv1 = conv2d_circular(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
...
class ResNetSemanticPositionCircular(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, num_location=9):
        super(ResNetSemanticPositionCircular, self).__init__()
        self.in_planes = 64
        ...
        # self.semantic_location = nn.Sequential(
        #     OrderedDict([
        #         ('conv5_4', conv2d_circular(512, 256, 3, 1, 1, 1, )),
...
def ResNet18PositionCircular(grid_size=3, num_class=10):
    print('Circular Padding!!')
    return ResNetPositionCircular(BasicBlock, [2, 2, 2, 2], num_classes=num_class, num_location=grid_size*grid_size)


def ResNet18SemanticPositionCircular(grid_size=3, num_class=11):
    print('Circular Padding!!')
    return ResNetSemanticPositionCircular(BasicBlock, [2,2,2,2], num_classes=num_class, num_location=grid_size*grid_size)
